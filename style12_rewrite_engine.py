...
ENGINE_VERSION = "v25-style12-gemini-rewrite"
...
def select_model(payload: dict[str, Any]) -> tuple[str, str]:
    ...
    return (os.getenv(env_name) or default).strip(), tier
...
def call_gemini_text(*, api_key: str, model: str, system: str, prompt: str, temperature: float) -> tuple[str, dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                raise RuntimeError(
                    ...
                    f"{detail_message or 'request failed'}"
                )
            text, meta = _extract_gemini_text(response.json())
            meta.update({"http_attempt": attempt, "model": model})
            return text, meta
            ...
            if attempt < retries:
                ...
                continue
            break
    raise RuntimeError(str(last_error or "Gemini rewrite request failed."))
...
def generate_style_options(raw_text: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    model, tier = select_model(payload)
